...
ControlInput AP::steer(const Spacecraft& craft_, const Vec2& portPosition) {
    ...
    if (status_ == AP_status::Approach) {
        ...
        finished_ = false;
        if (!isMaxRecorded_) {
            max_distance_ = (target_approach - craft_.position()).magnitude(); //Find the max distance for future normalization
            max_radian_error_ = radian_error;
            isMaxRecorded_ = true;
        }

        double kt = std::clamp(std::min((radian_error / max_radian_error_) , (craft_.velocity().magnitude() / 7)), 0.1, 0.7); //Thrust coefficient, sth that coorporates the radian_error and distance to the thrust provided
        ...
        if (double_abs(radian_error) < 5 && position_error.magnitude() >= 10 && craft_.velocity().magnitude() > 11) {
            ...
            kt = 1;
            double thrust_input = kt * position_error.magnitude() / max_distance_;
        ...
        if (double_abs(radian_error) > 0.01) {
            ...
            if (!isMaxRecorded_) {
                max_y_distance_ = double_abs(port.y - craft_.position().y);
            }
            double error_y = port.y - craft_.position().y;
            if (craft_.velocity().magnitude() < 2 && !finished_) {
                input.lateralThrottle += std::clamp(error_y / max_y_distance_, -0.7, 0.7);
            }
            ...
            max_distance_ = (target_approach - craft_.position()).magnitude();
            isMaxRecorded_ = false;
        }
        ...
        double radian_error = wrapAngle(desired_angle - craft_.angleRadians() );//makes sure radian_error is minimized to -180-180 deg
        // std::cout<<radian_error<<std::endl;
        double kt = std::clamp(std::min((position_error.magnitude() / max_distance_) , (craft_.velocity().magnitude() / (config_global.dockingLimits.safeSpeed * 2 / 5))), 0.1, 0.3); //Thrust coefficient, sth that coorporates the radian_error and distance to the thrust provided
