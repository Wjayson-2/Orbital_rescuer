...
void drawButtons(UIState& uiState) {

        uiState.timescalex1.draw();
        uiState.timescalex2.draw();
        uiState.timescalex5.draw();

}

void drawTimescale(TimeScale timescale, UIState& uiState) {
    std::string text;
    if (timescale == TimeScale::x1) {
        text = "x1";
        uiState.timescalex1.visible = true;
        uiState.timescalex2.visible = false;
        uiState.timescalex5.visible = false;
    }
    else if (timescale == TimeScale::x2) {
        text = "x2";
        uiState.timescalex2.visible = true;
        uiState.timescalex5.visible = false;
        uiState.timescalex1.visible = false;
    }
    else if (timescale == TimeScale::x5) {
        text = "x5";
        uiState.timescalex5.visible = true;
        uiState.timescalex2.visible = false;
        uiState.timescalex1.visible = false;
    }
...
void drawMainMenu(UIState& uiState) {
    drawStars();
    ...
    uiState.startButton.draw();
    uiState.quitButton.draw();
...
void drawMissionSelectMenu(UIState& uiState) {
    drawStars();
    ...
    uiState.mission1Button.draw();
    uiState.mission2Button.draw();
    uiState.mission3Button.draw();
    uiState.missionBackButton.draw();
}

void drawResultsMenu(const Simulation& simulation, UIState& uiState) {
    drawStars();
    ...
    uiState.retryButton.draw();
    uiState.statButton.draw();
    uiState.selectMissionButton.draw();
}
...
void drawStatsMenu(
    ...
    // Buttons
    uiState.retryButton_2.draw();
    uiState.selectMissionButton_2.draw();
    uiState.trailButton.draw();
}

void drawFlight(const Simulation& simulation,
                const ControlInput& controlInput,
                bool paused,
                UIState& uiState) {
    drawStars();
    drawStation(simulation.stationPosition(), simulation.portPosition());
    drawSpacecraft(simulation.craft(), controlInput);
    drawTelemetry(simulation);
    drawTimescale(simulation.gettimescale(), uiState);
    drawPlanet();
    drawHelp();
    drawFPS();
...
void drawTrailScreen(const Simulation& simulation, UIState& uiState) {
    drawStars();
    drawPlanet();
    drawStation(simulation.stationPosition(), simulation.portPosition());
    drawTrail(simulation);
    uiState.returnButton.draw();
}
...
void draw(const Simulation& simulation,
    ...
    switch (simulation.getGameStatus()) {
        case GameStatus::Flight:
            drawFlight(simulation, controlInput, paused, uiState);
            break;

        case GameStatus::MainMenu:
            drawMainMenu(uiState);
            break;

        case GameStatus::MissionSelect:
            drawMissionSelectMenu(uiState);
            break;

        case GameStatus::Results:
            drawResultsMenu(simulation, uiState);
            break;

        case GameStatus::Stats: {
            const MissionStats stats = simulation.getMissionStats();

            drawStatsMenu(
                simulation.getState() == MissionState::Docked,
                config_global.name,
                config_global.startFuel,
                stats.finalFuel,
                stats.missionTime,
                stats.maxSpeed,
                stats.impactSpeed,
                config_global.dockingLimits.safeSpeed,
                stats.hullIntegrity,
                simulation.score(),
                uiState
            );
        ...
        case GameStatus::Trail:
            drawTrailScreen(simulation, uiState);
            break;
    }
}
